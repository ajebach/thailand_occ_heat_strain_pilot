...
df3 <- df2 %>%
  ...
  summarise(avg_tc = mean(avg_tc, na.rm = TRUE)) %>%
  select(cond, interval, avg_tc)

df1 <- df1 %>%
  mutate(cond = relevel(as.factor(cond), ref = 'vest'))

fit <- gam(tc ~ s(interval, by = cond, k = 21) + cond, data = df1)
summary(fit)
...
ref <- ref_grid(fit, at = list(interval = seq(min(df1$interval), max(df1$interval), by = 1),
                               cond = c('con','fan','vest'), type = 'transform'))
...
my_palette <- c("con" = "#D53E4F", "vest" = "#41AB5D", "fan" = "#3288BD")
...
post_lunch <- gam(tc ~ s(interval, by = cond, k = 14) + cond, data = dsub_tc)
summary(post_lunch)

print(confint(post_lunch, level = 0.95))

dsub_tc <- dsub_tc %>%
  mutate(cond = relevel(cond, ref = 'fan'))

post_lunch_comparison <- gam(tc ~ s(interval, by = cond, k = 14) + cond, data = dsub_tc)
summary(post_lunch_comparison)

print(confint(post_lunch_comparison, level = 0.95))

ref <- ref_grid(post_lunch, at = list(interval = seq(min(dsub_tc$interval), max(dsub_tc$interval), by = 1),
                                      cond = c('con', 'fan', 'vest')))
...
plot_data_tc_lunch$cond <- factor(plot_data_tc_lunch$cond, levels = c("con", "vest", "fan"))
df3_lunch$cond <- factor(df3_lunch$cond, levels = c("con", "vest", "fan"))
...
fit <- gam(avg_hr ~ s(interval, by = cond, k = 24) + cond, data = df5)
...
post_lunch <- gam(avg_hr ~ s(interval, by = cond, k = 18) + cond, data = dsub_hr)

summary(post_lunch)

print(confint.gam(post_lunch, level = 0.95))

ref <- ref_grid(post_lunch, at = list(interval = seq(min(dsub_hr$interval), max(dsub_hr$interval), by = 1),
                                      cond = c('con', 'fan', 'vest'), type = 'transform'))
...
plot_data_hr_lunch$cond <- factor(plot_data_hr_lunch$cond, levels = c("con", "vest", "fan"))
df5_means_lunch$cond <- factor(df5_means_lunch$cond, levels = c("con", "vest", "fan"))
